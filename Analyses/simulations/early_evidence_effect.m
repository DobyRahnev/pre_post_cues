%early_evidence_effect

clear
close all

N = 30000;
weights = [7:-1:2,ones(1,24)];
...
all_weights = ones(N,30);
for frame=1:6
    all_weights(cues==1 & orientationAngle(:,frame)' < 0, frame) = weights(frame);
    all_weights(cues==2 & orientationAngle(:,frame)' > 0, frame) = weights(frame);
end
orientationAnglePerceived = orientationAngle.*all_weights;
