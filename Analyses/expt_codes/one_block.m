function data = one_block(p, number_trials, cue_order)
...
for trial=1:number_trials
    ...
    if rand < .25 %present neutral cue on 1/4th of the trials
        cue_type(trial) = 3; %neutral
    else
        if rand < .5
            cue_type(trial) = 1; %CCW/left
        else
            cue_type(trial) = 2; %CW/right
        end
    end
    ...
    if cue_order == 1 %'before'
        ...
        Screen('FillOval',p.window,131,[p.width/2-radius2,p.height/2-radius2,p.width/2+radius2,p.height/2+radius2] );
        %Screen('FillOval', p.window,131, [p.width/2-p.stimInner-10,p.height/2-p.stimInner-10,p.width/2+p.stimInner+10,p.height/2+p.stimInner+10]);%encircles cue
    ...
    for frame = 1:p.frames_per_trial
        ...
        %Choose orientation
        while 1
            orientationAngle(trial,frame) = 22.5*randn + (2*stim_orientation(trial)-3) * p.orientationOffset;
